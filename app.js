...
const CATEGORIES = {
  ...
  tension: {
    ...
    steps: [
      {
        title: "Напряжение как стратегия",
        comment: "Напряжение редко случайно — обычно оно что-то выполняет: держит в готовности к опасности, не даёт потерять контроль, помогает «не выглядеть слабым», подготовиться к критике или не столкнуться с неприятными чувствами. Полезнее спросить не «почему я такой?», а какую функцию оно выполняет.",
        questions: [
          "От чего меня пытается защитить это напряжение?",
          "Какую функцию оно выполняет — готовность, контроль, защиту, «понравиться», «не ошибиться»?"
        ]
      },
      {
        title: "Карта напряжения",
        comment: "Разберите конкретный эпизод по схеме: ситуация → тело → мысль → опасение → поведение → последствие.",
        questions: [
          "Где я был, когда напрягся?",
          "Где именно в теле напряжение — челюсть, плечи, живот, грудь, лицо?",
          "Что я говорил себе в этот момент?",
          "Что плохого могло случиться, если бы я расслабился?",
          "Что я сделал — замолчал, сжался, начал себя контролировать, ушёл в телефон?",
          "Помогло ли это в моменте? Помешало ли потом?"
        ]
      },
      {
        ...
        questions: [
          "Что, по ощущению, случится, если я расслаблюсь?",
